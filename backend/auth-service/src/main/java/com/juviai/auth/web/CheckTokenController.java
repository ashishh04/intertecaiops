...
public class CheckTokenController {
    ...
    private ResponseEntity<Map<String, Object>> doIntrospection(String tokenValue) {
        ...
        try {
            auth = authorizationService.findByToken(tokenValue, OAuth2TokenType.ACCESS_TOKEN);
        } catch (Exception ignored) {
            // Some persisted authorizations may fail to deserialize (e.g., due to Spring Security Jackson allowlist).
            // In that case, fall back to a simpler DB existence check below.
        }
        ...
        Map<String, Object> tokenMetadata = at.getMetadata();
        Map<String, Object> authAttributes = auth.getAttributes();

        Object username = tokenMetadata.get("username");
        if (username == null) {
            username = tokenMetadata.get(OAuth2ParameterNames.USERNAME);
        }
        if (username == null) {
            username = authAttributes.get("username");
        }
        if (username == null) {
            username = authAttributes.get(OAuth2ParameterNames.USERNAME);
        }
        if (username != null) {
        ...
        Object roles = tokenMetadata.get("roles");
        if (roles == null) {
            roles = authAttributes.get("roles");
        }
        if (roles != null) {
            body.put("roles", roles);
        }

        Object tenantId = tokenMetadata.get("tenant_id");
        if (tenantId == null) {
            tenantId = authAttributes.get("tenant_id");
        }
        if (tenantId != null) {
            body.put("tenant_id", tenantId);
        }

        Object userId = tokenMetadata.get("userId");
        if (userId == null) {
            userId = authAttributes.get("userId");
        }
        if (userId != null) {
